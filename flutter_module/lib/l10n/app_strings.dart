...
class AppStrings {
  ...
  String get agentServerSubtitle => _isZh
  ...
  String get serverAddressNeedPort => _isZh
      ? '请包含端口号（如：:5000）'
  ...
  String get permissionDialogTitle =>
      _isZh ? '需要权限' : 'Permissions Required';
  String get permissionDialogDescription => _isZh
  ...
  String get accessibilityPermissionDescription => _isZh
      ? '允许应用响应系统事件。'
      : 'Allows the app to respond to system events.';
  String get batteryPermissionTitle =>
      _isZh ? '电池优化' : 'Battery Optimization';
  String get batteryPermissionDescription => _isZh
      ? '防止系统停止应用。'
      : 'Prevents the system from stopping the app.';
  ...
  String devServerStartLabel({required bool compact}) => _isZh
      ? (compact ? '启动' : '启动 DevServer')
      : (compact ? 'Start' : 'Start DevServer');
  String devServerStopLabel({required bool compact}) => _isZh
      ? (compact ? '停止' : '停止 DevServer')
      : (compact ? 'Stop' : 'Stop DevServer');
  String get devServerDesktopOnly => _isZh
      ? 'DevServer 模式请在电脑端发送任务。'
      : 'DevServer mode: send tasks from the desktop.';

  List<String> get initialSuggestions => _isZh
      ? const [
          '📷 打开相机并拍一张照片',
          '📅 创建明天上午的会议提醒',
          '🛫 查询北京飞上海的机票',
  ...
  String get welcomeMessage => _isZh
  ...
  String get summaryInProgress => _isZh ? '正在为您总结...' : 'Preparing your summary...';
  String get companionModeEntered => _isZh
  ...
  String get connectFailure => _isZh
  ...
  String get operationInProgress => _isZh
      ? '⚠️ 当前有操作正在进行，请稍后再试。'
  ...
  String devServerStartSuccess(String address) => _isZh
  ...
  String devServerStartFailure(String message) =>
      _isZh ? '❌ DevServer 启动失败: $message' : '❌ DevServer failed to start: $message';
  ...
  String devServerStopFailure(String message) =>
      _isZh ? '❌ DevServer 停止失败: $message' : '❌ DevServer failed to stop: $message';
